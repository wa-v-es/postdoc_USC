...
outps=`echo $outps | sed 's/\.ps/_beamRECORD\.ps/'`
...
gmt psbasemap $sac_range $sac_frame -B${time_border}:"Time after origin (s)":/1:"Distance (deg)":WSen -K -Y1c -X1.5c > $outps
...
#XF SLOW - START  (3) ----------
#-------------------------------------------------
beam_slow_grid=$temps"TEMP_BEAM_SLOW_GRID_"$proc_id
frame="-JX17c/5.0c"
contcut_beam=50
...
yoffset1=8.5c
yoffset_map="-3c"
beam_grid_max=`gmt grdinfo $beam_slow_grid | gawk '/z_max/ {print $5}'`
#beam_grid_inc=`echo $beam_grid_max | gawk '{printf "%.2f", $1/2.5}'`
beam_grid_inc=`echo $beam_grid_max | gawk '{printf "%.8f", $1/2.5}'`
range_vars=(`gmt grdinfo $beam_slow_grid | grep x_min -A1 | awk '{print $3, $5}'`)
#range_grd="-R"${range_vars[0]}"/"${range_vars[1]}"/"${range_vars[2]}"/"${range_vars[3]}
range_grd="-R"$plotcutmin"/"$plotcutmax"/"${range_vars[2]}"/"${range_vars[3]}
...
gmt grdimage $beam_slow_grid $range_grd $frame -C$temps"TEMP_BEAM.cpt_"$proc_id -B$time_border:"Time (sec)":/2:"Slowness (s/deg)":Wsen -X$xoffset1 -Y$yoffset1 -O -K >> $outps
#gmt grdcontour $beam_slow_grid $range_grd $frame -W0.02c,darkgray -C$beam_grid_inc -Q$contcut_beam -K -O >> $outps
gmt psscale -C$temps"TEMP_BEAM.cpt_"$proc_id -D$scale_var -B$beam_grid_inc -O -K >> $outps
...
#XF BAZ - START (4) ----------
#-------------------------------------------------
beam_baz_grid=$temps"TEMP_BEAM_BAZ_GRID_"$proc_id
xoffset1=0c
yoffset1=5.5c
#beam_grid_max=`gmt grdinfo $beam_baz_grid | gawk '/z_max/ {print $5}'`
#beam_grid_inc=`echo $beam_grid_max | gawk '{printf "%.f", $1/5}'`
range_vars=(`gmt grdinfo  $beam_baz_grid | grep x_min -A1 | awk '{print $3, $5}'`)
#range_grd="-R"${range_vars[0]}"/"${range_vars[1]}"/"${range_vars[2]}"/"${range_vars[3]}
range_grd="-R"$plotcutmin"/"$plotcutmax"/"${range_vars[2]}"/"${range_vars[3]}


gmt grdimage $beam_baz_grid $range_grd $frame -C$temps"TEMP_BEAM.cpt_"$proc_id -B$time_border:"Time (sec)":/20g40:"Back-azimuth (deg)":Wsen -Y$yoffset1 -O -K >> $outps
#gmt grdcontour $beam_baz_grid $range_grd $frame -W0.02c,darkgray -C$beam_grid_inc -Q$contcut_beam -K -O >> $outps
...
gmt pstext $frame $range_text -F+jLM+a0+f11,0,black -N -O -K <<EOF >> $outps
