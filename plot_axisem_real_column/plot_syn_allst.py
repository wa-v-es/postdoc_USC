...
########
input_dir='/Users/keyser/Research/axisem/forPC_output/output_pure_explosion/input'
syn_dir = '/Users/keyser/Research/axisem/forPC_output/output_pure_explosion/stations/5deg_array/sac/'
info_5deg = np.loadtxt(input_dir+'/5deg.txt', dtype=str, skiprows=3)
...
loc_leaf = source_yaml['list_of_sources'][0]['source_2024']['location']
...
for ist, st in enumerate(info_5deg):
    ...
    origin_time = get_sac_reftime(st_syn[0].stats.sac)
    # st_syn[0].stats.starttime=origin_time
    #### to align the seismograms

    model = TauPyModel(model="ak135")
    try:
        arrivals = model.get_travel_times(source_depth_in_km=st_syn[0].stats.sac['evdp']/1000,distance_in_degree=st_syn[0].stats.sac['gcarc'],phase_list=["P"])
        arr_P=arrivals[0]

        st_syn.trim(endtime=origin_time+1200)
...
for j in range(3):
    ...
    if N_new >= len(syn_all):  # last bit
        st_temp = syn_all[N_old:]
    else:
        st_temp = syn_all[N_old:N_new]  # current chunk
    N_old = N_new
    print(st_temp,'\n')
    print('################################')
    ...
    ax[j].set_ylim(12, 0)
    ax[j].set_xlim(0,80)
    ax[j].set_yticks(np.linspace(0,12,13))
    ...
    ax[j].xaxis.set_major_locator(MultipleLocator(20))
    ...
    for i in range(len(st_temp)):
        ...
        # arrivals = model.get_travel_times(source_depth_in_km=auto.stats.sac['evdp'],distance_in_degree=auto.stats.sac['gcarc'],phase_list=["PP"])
        # arr_PP=arrivals[0]
        auto.trim(starttime=origin_time+arr_P.time-20,endtime=origin_time+arr_P.time+60)
        ax[j].scatter(20, i+1,s=5,marker='o',facecolor='maroon', edgecolor='black',alpha=.5,linewidth=.15,zorder=10)
        ...
        l1,=ax[j].plot(time,2*auto.data + i+1,  lw=0.4, color='teal',label='Syn')
        ####
...
plt.title('10 sec seis for pure 5Mw explosion - iasp91',loc='center')
fig.text(0.52, 0.03, 'Time after event (s)',fontsize=11, ha='center', va='center')
plt.show()
#
plt.savefig('10sec_syn_all_iasp91.png',dpi=300,bbox_inches='tight', pad_inches=0.1)
sys.exit()
############
